...
export async function loadFleetBackendsFromMcpuConfig(fleetConfig, logger) {
    const configPath = fleetConfig.mcpu_generated_config ??
    ...
    try {
        ...
        logger.warn(`Fleet ingestion: could not read MCPU config at ${configPath}: ${err instanceof Error ? err.message : String(err)}`);
        return result;
    }
    ...
    logger.info(`Fleet ingestion: ${Object.keys(result.backends).length} backends loaded from ${configPath} (${result.skipped.length} skipped)`);
    return result;
}
