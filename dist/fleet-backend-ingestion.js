...
function makeUniqueNamespace(baseNamespace, backendName, usedNamespaces, logger) {
    ...
    if (priorBackend && namespace !== fallbackNamespace) {
        logger.warn(`Fleet ingestion: namespace collision for "${backendName}" and "${priorBackend}"; using namespace "${namespace}"`);
    }
...
function normalizeConfigMap(parsed) {
    if (!isRecord(parsed))
        return {};
    const maybeWrapped = parsed.mcpServers;
    if (isRecord(maybeWrapped)) {
        return Object.fromEntries(Object.entries(maybeWrapped).filter(([, value]) => isRecord(value)));
    }
    return Object.fromEntries(Object.entries(parsed).filter(([, value]) => isRecord(value)));
}
async function readConfigSource(source, logger) {
    try {
        const content = await readFile(source.path, "utf-8");
        return normalizeConfigMap(JSON.parse(content));
    }
    catch (err) {
        logger.warn(`Fleet ingestion: could not read MCPU config at ${source.path}: ${err instanceof Error ? err.message : String(err)}`);
        return undefined;
    }
}
 ...
 */
export async function loadFleetBackendsFromMcpuConfig(fleetConfig, logger) {
    const generatedPath = fleetConfig.mcpu_generated_config ??
        join(homedir(), ".config", "mcpu", "config.generated.json");
    const staticPath = fleetConfig.mcpu_static_config ??
        join(homedir(), ".config", "mcpu", "config.json");
    const configSources = [
        { path: generatedPath, source: "fleet-mcpu-generated" },
        ...(fleetConfig.ingest_static_mcpu_config
            ? [{ path: staticPath, source: "fleet-mcpu-static" }]
            : []),
        ...fleetConfig.additional_mcpu_configs.map((path) => ({
            path,
            source: "fleet-mcpu-additional",
        })),
    ];
    const result = {
        backends: {},
    ...
    for (const source of configSources) {
        ...
        for (const [name, entry] of Object.entries(rawConfig)) {
            ...
            const namespace = makeUniqueNamespace(`${namespacePrefix}${sanitiseName(name)}`, name, usedNamespaces, logger);
            if (type === "stdio") {
                if (!entry.command) {
    ...
    logger.info(`Fleet ingestion: ${Object.keys(result.backends).length} backends loaded from ${configSources.length} MCPU config source(s) (${result.skipped.length} skipped)`);
    return result;
}
//# sourceMappingURL=fleet-backend-ingestion.js.map